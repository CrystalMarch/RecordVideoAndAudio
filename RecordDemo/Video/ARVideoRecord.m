...
@interface ARVideoRecord()<AVAudioRecorderDelegate>
...
- (void)initData {
    ...
    // 创建队列
    self.videoQueue = dispatch_queue_create("ar.video.queue", NULL);
    self.audioQueue = dispatch_queue_create("ar.audio.queue", NULL);
    
    // 设置输出分辨率
    self.outputSize = CGSizeMake(SCREEN_WIDTH+1, SCREEN_HEIGHT+1);
...
- (void)updateProgress{
    ...
    dispatch_async(dispatch_get_main_queue(), ^{
        if (self.delegate && [self.delegate respondsToSelector:@selector(updateRecordingProgress:)]) {
            [self.delegate updateRecordingProgress:_recordTime/VIDEO_RECORD_MAX_TIME ];
        }
    });
